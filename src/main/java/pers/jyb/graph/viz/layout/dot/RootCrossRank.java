...
import pers.jyb.graph.viz.layout.dot.MinCross.ClusterOrder;
...
class RootCrossRank implements CrossRank {
  ...
  RootCrossRank(DrawGraph drawGraph) {
    ...
    this.root = new BasicCrossRank(drawGraph.getGraphviz(), Boolean.TRUE);
  ...
  void removeEdge(DLine line) {
    digraphProxy.removeEdge(line);
  }

  Iterable<DLine> inAdjacent(DNode node) {
    return digraphProxy.inAdjacent(node);
  }

  Iterable<DLine> outAdjacent(DNode node) {
  ...
  BasicCrossRank expand(ExpandInfoProvider expandInfoProvider) {
    ...
    for (DNode expandNode : expandNodes) {
      ...
      // sync node id that rank index large than insert index
  ...
  private boolean isCross(DLine line1, DLine line2) {
    ...
    if (u == v || u == y || x == v || x == y) {
      ...
      if (u == v) {
        Port up = getLineEndPointPort(u.getNode(), line1.getLine());
        Port vp = getLineEndPointPort(v.getNode(), line2.getLine());

        if (x.getRank() == u.getRank()) {
          return PortHelper.portCompare(up, vp) < 0 == getRankIndex(x) < getRankIndex(y);
        }
        int ul = PortHelper.crossPortNo(up);
        int vl = PortHelper.crossPortNo(vp);
        return locationTag(ul, vl) * locationTag(y, x)
            + locationTag(vl, ul) * locationTag(x, y) == 1;
      }

      Port xp = getLineEndPointPort(x.getNode(), line1.getLine());
      Port yp = getLineEndPointPort(y.getNode(), line2.getLine());

      if (u.getRank() == x.getRank()) {
        return PortHelper.portCompare(xp, yp) < 0 == getRankIndex(u) < getRankIndex(v);
      }
      int xl = PortHelper.crossPortNo(xp);
      int yl = PortHelper.crossPortNo(yp);
  ...
  private int locationTag(int o1, int o2) {
    return o1 < o2 ? 1 : 0;
  }

  private Port getLineEndPointPort(Node n, Line line) {
    return PortHelper.getLineEndPointPort(n, line, drawGraph);
  }
