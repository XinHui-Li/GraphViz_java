...
public class PortNodeSizeExpander extends NodeSizeExpander {
  ...
  private void init(DrawGraph drawGraph) {
    ...
    TreeSet<SelfLine> lines = sortSelfLine(drawGraph, nodeShape);

    // Calculate all axes offset and update node volume
    selfLinePoints(drawGraph, nodeShape, lines);
  }

  private void selfLinePoints(DrawGraph drawGraph, NodeShape nodeShape, TreeSet<SelfLine> lines) {
    ...
    for (SelfLine selfLine : lines) {
      ...
      do {
        ...
        FlatPoint point = PortHelper.notFlipEndPoint(null, port, nodeShape, drawGraph, node);

        if (center == null) {
          center = new FlatPoint(node.getX(), node.getY());
        }
        double d = FlatPoint.twoFlatPointDistance(point, center);
        FlatPoint flatPoint = Vectors.addVector(center, Vectors
            .multipleVector(Vectors.subVector(point, center), (d + len) / d));
        if (port == midPort) {
  ...
  private TreeSet<SelfLine> sortSelfLine(DrawGraph drawGraph, NodeShape nodeShape) {
    ...
    for (int i = 0; i < node.getSelfLoopCount(); i++) {
      ...
      String tailLabelPort = lineAttrs.getTailLabelPort();
      String headLabelPort = lineAttrs.getHeadLabelPort();

      FlatPoint tailPoint = PortHelper
          .endPoint(tailLabelPort, lineAttrs.getTailPort(), node.getNode(), drawGraph, node);
      FlatPoint headPoint = PortHelper
          .endPoint(headLabelPort, lineAttrs.getHeadPort(), node.getNode(), drawGraph, node);
  ...
  private void addPoint(LineDrawProp lineDrawProp, FlatPoint point) {
  ...
  private void addLabel(DLine line, LineDrawProp lineDrawProp, int labelIdx) {
    ...
    if (point.getX() == node.getX()) {
      ...
      double d = FlatPoint.twoFlatPointDistance(point, center);
      labelCenter = Vectors.addVector(center, Vectors
          .multipleVector(Vectors.subVector(point, center), (d + halfHeight) / d));
    }
  ...
  private void refreshVolume(FlatPoint point) {
    refreshVolume(point.getX(), point.getY());
  }

  private void refreshVolume(double x, double y) {
