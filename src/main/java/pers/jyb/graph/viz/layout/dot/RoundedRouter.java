  ...
  private MultiBezierCurve getCorner(ThroughPoint lt, ThroughPoint p1, ThroughPoint p2,
    ...
    if (d1 < radian || d2 < radian) {
      MultiBezierCurve curves = Curves.fitCurves(Arrays.asList(p1, p3),
                                                 lt != null ? Vectors.subVector(p1, lt) : null,
                                                 rt != null ? Vectors.subVector(p3, rt) : null, 0);

      return checkFixBox(lineRouterBoxes, curves);
    }

    FlatPoint dir = Vectors.subVector(p2, p1);
    double ratio = (d1 - radian) / d1;
    FlatPoint v1 = Vectors.addVector(p1, Vectors.multipleVector(dir, ratio));

    ratio = (d1 - radian / 2) / d1;
    FlatPoint v2 = Vectors.addVector(p1, Vectors.multipleVector(dir, ratio));

    dir = Vectors.subVector(p2, p3);
    ratio = (d2 - radian) / d2;
    FlatPoint v4 = Vectors.addVector(p3, Vectors.multipleVector(dir, ratio));

    ratio = (d2 - radian / 2) / d2;
    FlatPoint v3 = Vectors.addVector(p3, Vectors.multipleVector(dir, ratio));
