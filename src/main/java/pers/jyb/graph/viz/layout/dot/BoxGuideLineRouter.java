...
abstract class BoxGuideLineRouter extends AbstractDotLineRouter {
  ...
  protected void handleSameEndpointParallelLines(List<DLine> parallelLines) {
    ...
    FlatPoint fromPoint = getPortPoint(from, fromPort);
    FlatPoint toPoint = getPortPoint(to, toPort);
  ...
  private void sameRankParallelLineDraw(ShapePosition shapePosition,
    ...
    for (int j = 0; j < parallelLines.size(); j++) {
      ...
      // 分为 flat label node在两个节点之间 和 在两个节点所在层级相邻层级 两种情况
      if (alternateDraw) {
        // label永远摆在线的上端，根据中间的对称轴一上一下
        if (j % 2 == 0) {
          if (upBaseLine == null) {
  ...
  private FlatPoint getPortPoint(DNode node, Port port) {
    if (port == null) {
  ...
  private void flatLineBoxes(DLine line,
                             List<RouterBox> lineRouterBoxes,
                             RankNode rankNode,
                             RankNode pre,
                             RankNode next,
  ...
  private void lineCompute(Line line, LineDrawProp lineDrawProp,
    ...
    List<RouterBox> originRouterBoxes = splitPortBox(line, lineRouterBoxes,
          ...
          if (originRouterBox.getNode() == from) {
            ...
            if (i > 1) {
              if (curveLine()) {
                ...
                if (i < throughPoints.size()) {
                  rightTangent = Vectors.subVector(throughPoints.get(i - 1), throughPoints.get(i));
                }
            ...
            if (i < throughPoints.size() - 2) {
              if (curveLine()) {
                ...
                if (i >= 0) {
                  leftTangent = Vectors.subVector(throughPoints.get(i + 1), throughPoints.get(i));
                }
  ...
  private int throughPointCompute(List<ThroughPoint> throughPoints,
    ...
    for (int i = boxStartIndex; i <= boxEndIndex; i++) {
      ...
      if (vertical) {
        ...
        // double计算会有一定的精确度误差，前后移位1px修复误差
  ...
  /*
   * 当前节点在左右两节点间创建box，如果当前节点是虚拟的，会跳过左边或者右边的一个存在交叉的虚拟节点
   */
  ...
  // 寻找节点的左边或者右边节点，以是否交叉的原则选择跳过邻居节点
  ...
  private List<RouterBox> splitPortBox(Line line, List<RouterBox> routerBoxes, boolean vertical) {
    ...
    List<RouterBox> splitRouterBoxes = splitPortBox(firstRouterBox, line, vertical, true);
    ...
    splitRouterBoxes = splitPortBox(lastRouterBox, line, vertical, false);
    if (CollectionUtils.isNotEmpty(splitRouterBoxes)) {
  ...
  private List<RouterBox> splitPortBox(RouterBox routerBox, Line line, boolean vertical,
                                       boolean isFrom) {
    DNode node = routerBox.getNode();
    if (node == null) {
      return Collections.emptyList();
    ...
    Port port = PortHelper.getLineEndPointPort(node.getNode(), line, drawGraph);
    if (port == null) {
      return Collections.emptyList();
    }

    FlatPoint point = PortHelper.getPortPoint(line, node, drawGraph);
    ...
    if (point.getY() < node.getY() || needHandleEqual) {
      if (vertical) {
