...
abstract class AbstractCoordinate {
  ...
  protected DotAttachment dotAttachment;

  protected EdgeDedigraph<DNode, DLine> proxyDigraph;
  ...
  protected RankContent networkSimplex(DotDigraph auxGraph) {
    FeasiableTree feasiableTree = new FeasiableTree(auxGraph);
    NetworkSimplex networkSimplex = new NetworkSimplex(feasiableTree, nslimit, false,
  ...
  protected int crossLineLimit(DLine line) {
    ...
    double fromOffset = dotAttachment.nodeHorPortOffset(from.getNode(), line.getLine(), from);
    double toOffset = dotAttachment.nodeHorPortOffset(to.getNode(), line.getLine(), to);
    return (int) (fromOffset - toOffset);
  }
