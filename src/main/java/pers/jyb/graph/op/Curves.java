...
public final class Curves {
	...
	public static ThirdOrderBezierCurve divideThirdBesselCurve(double t, boolean isFirstHalf,
		...
		FlatPoint intersection = Vectors.addVector(
				bezierCurve.v2,
				Vectors.multipleVector(Vectors.subVector(bezierCurve.v3, bezierCurve.v2), t)
		);
		ThirdOrderBezierCurve childCurve = new ThirdOrderBezierCurve();

		if (isFirstHalf) {
			childCurve.v1 = bezierCurve.v1;
			childCurve.v2 = Vectors.addVector(
					bezierCurve.v1,
					Vectors.multipleVector(Vectors.subVector(bezierCurve.v2, bezierCurve.v1), t)
			);
			childCurve.v3 = Vectors.addVector(
					childCurve.v2,
					Vectors.multipleVector(Vectors.subVector(intersection, childCurve.v2), t)
			);
			childCurve.v4 = dividePoint;
		} else {
			childCurve.v1 = dividePoint;
			childCurve.v3 = Vectors.addVector(
					Vectors.multipleVector(Vectors.subVector(bezierCurve.v3, bezierCurve.v4), 1 - t),
					bezierCurve.v4
			);
			childCurve.v2 = Vectors.addVector(
					intersection,
					Vectors.multipleVector(Vectors.subVector(childCurve.v3, intersection), t)
			);
	...
	private static <F extends FlatPoint> MultiBezierCurve fitCurves(int first, int last, double error,
		...
		if (last - first + 1 == 2) {
			...
			thirdOrderBezierCurve.v2 = Vectors.addVector(
					thirdOrderBezierCurve.v1,
					Vectors.vectorScale(leftTangent, distance)
			);
			thirdOrderBezierCurve.v3 = Vectors.addVector(
					thirdOrderBezierCurve.v4,
					Vectors.vectorScale(rightTangent, distance)
			);
	...
	private static <F extends FlatPoint> ThirdOrderBezierCurve generateBezier(List<F> points,
		...
		for (int i = 0; i < size; i++) {
			A[i] = new FlatPoint[2];
			A[i][0] = Vectors.vectorScale(leftVector, B1(pointChordLens[i]));
			A[i][1] = Vectors.vectorScale(rightVector, B2(pointChordLens[i]));
		}
		...
		for (int i = 0; i < size; i++) {
			C[0][0] += Vectors.vectorDot(A[i][0], A[i][0]);
			C[0][1] += Vectors.vectorDot(A[i][0], A[i][1]);

			C[1][0] = C[0][1];
			C[1][1] += Vectors.vectorDot(A[i][1], A[i][1]);

			FlatPoint tmp = Vectors.subVector(
					points.get(first + i),
					Vectors.addVector(
							Vectors.multipleVector(firstPoint, B0(pointChordLens[i])),
							Vectors.addVector(
									Vectors.multipleVector(firstPoint, B1(pointChordLens[i])),
									Vectors.addVector(
											Vectors.multipleVector(lastPoint, B2(pointChordLens[i])),
											Vectors.multipleVector(lastPoint, B3(pointChordLens[i]))
									)
							)
					)
			);

			X[0] += Vectors.vectorDot(A[i][0], tmp);
			X[1] += Vectors.vectorDot(A[i][1], tmp);
		}
		...
		if (alphaL < epsilon || alphaR < epsilon) {
			...
			bezierCurve.v2 = Vectors.addVector(bezierCurve.v1, Vectors.vectorScale(leftVector, dist));
			bezierCurve.v3 = Vectors.addVector(bezierCurve.v4, Vectors.vectorScale(rightVector, dist));
		} else {
			/*
			 ...
			 */
			bezierCurve.v2 = Vectors.addVector(bezierCurve.v1, Vectors.vectorScale(leftVector, alphaL));
			bezierCurve.v3 = Vectors.addVector(bezierCurve.v4, Vectors.vectorScale(rightVector, alphaR));
		}
	...
	private static <F extends FlatPoint> double computeMaxError(List<F> points, int first, int last,
		...
		for (int i = first + 1; i < last; i++) {
			...
			FlatPoint v = Vectors.subVector(p, points.get(i));
	...
	private static <F extends FlatPoint> FlatPoint computeLeftTangent(List<F> points) {
		return Vectors.twoPointUnitVector(points.get(1), points.get(0));
	}

	// right point vector, determined by pre point
	private static <F extends FlatPoint> FlatPoint computeRightTangent(List<F> points) {
		return Vectors.twoPointUnitVector(points.get(points.size() - 2), points.get(points.size() - 1));
	}
	...
	private static <F extends FlatPoint> FlatPoint computeCenterTangent(List<F> points,
	                                                                    int centerIndex) {
		FlatPoint v1 = Vectors.subVector(points.get(centerIndex - 1), points.get(centerIndex));
		FlatPoint v2 = Vectors.subVector(points.get(centerIndex), points.get(centerIndex + 1));
		return Vectors.unitVector((v1.getX() + v2.getX()) / 2, (v1.getY() + v2.getY()) / 2);
	}
	...
	public static class ThirdOrderBezierCurve implements Serializable {
		...
		public void adjust(double v2AdjustRatio, double v3AdjustRatio) {
			this.v2 = Vectors.addVector(v1, Vectors.vectorScale(Vectors.subVector(v2, v1),
			                                                    FlatPoint.twoFlatPointDistance(v2, v1)
					                                                    * v2AdjustRatio));
			this.v3 = Vectors.addVector(v4, Vectors.vectorScale(Vectors.subVector(v3, v4),
